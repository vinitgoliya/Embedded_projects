...
with system clock faster than 40 MHz, 3 CCLKs are proposed.
 ...
 void init_PLL_OSCR(void)  // with crystal oscilator
{
    // Step 1: Configure PLL
    PLL0CFG = 0x24;       // M=5, P=2 => 12MHz x 5 = 60MHz
    PLL0CON = 0x01;       // Enable PLL
    PLL0FEED = 0xAA;
    PLL0FEED = 0x55;

    // Step 2: Wait for PLL to lock
    while (!(PLL0STAT & (1 << 10)));  // Wait for PLOCK bit

    // Step 3: Connect PLL as system clock
    PLL0CON = 0x03;       // Enable and connect PLL
    PLL0FEED = 0xAA;
    PLL0FEED = 0x55;

    // Step 4: Set peripheral clock (optional)
