...
static int strlength(unsigned char *str)
{
    int len = 0;
    while (str[len] != '\0')
    {
...
void Set_Lcd_String_Scrolling(u8_t *str)
{
    ...
    Set_Lcd_String(str);         // Display the full string
    delay(5000);                // Initial delay before scrolling
	  //length = strlength((unsigned char *)str);

    for ( i = 0; i <= 16; i++) // Shift 16 times for full scroll
    {
        Set_Lcd_Command(0x1C);   // Shift entire display to the right
			  vTaskDelay(500);
        //delay(10000);           // Delay between shifts
    }
		//Set_Lcd_String_Scrolling(str);
}
